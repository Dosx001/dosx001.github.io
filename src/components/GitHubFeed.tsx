...
const GitHubFeed = () => {
  ...
  const formatDate = (datetime: string) => {
    ...
    let time: string;
    if (hour === 0) time = `12:${min} AM`;
    else if (hour < 12) time = `${hour}:${min} AM`;
    else if (hour === 12) time = `12:${min} PM`;
  ...
  if (error) {
    return <div id="git">Error: {error}</div>;
  }
  if (!isLoaded) {
    return <div id="git">Loading...</div>;
  }
  return (
    <div id="git">
